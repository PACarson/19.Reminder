 ...
 *
 * === 2026-07-22 更新：V1 完全退役 + 同类风险排查 ===
 *
 * Carson 实测满意，批准彻底退役/合并 25_ReminderEngine.gs（不再只是
 * "观察期保留"）。同时，Overdue 阶段出现"5分钟内提醒2次、持续不停"的
 * 异常，排查后发现极可能是同一类问题的另一个实例：本项目还有
 * 12_QueryEngine.gs（2026-07-06 已被 22_QueryEngine.gs 取代，文件头
 * 明确写着"[原 12_QueryEngine.gs]"）——12_ 版本完全没有
 * reminder_count/last_reminder_at 的定点合并逻辑（22_ 版本这部分是
 * 2026-07-11 才加的），如果 12_ 这个旧文件还留在实际部署里，跟 22_ 一样
 * 声明了全局的 var QueryEngine，会产生跟这次 ReminderEngine 完全同类的
 * 冲突——不管最终是哪一份定义"赢"，只要 12_ 有任何影响，
 * getPendingTasks() 返回的 task 对象上 last_reminder_at 就可能读不到
 * 正确值，Overdue 阶段的间隔判断（"距上次提醒是否超过 interval_minutes"）
 * 会因为读到空值而永远判定"从没提醒过"，等于每次 5 分钟轮询都重新触发一次
 * ——这跟 Carson 观察到的症状完全吻合。
 *
 * 决定：这次一并清理全部同类风险文件，不再逐个发现逐个修：
 *   - 25_ReminderEngine.gs（V1）：彻底删除，不再保留。核心逻辑
 *     （_shouldRemind/REMINDER_INTERVAL_HOURS 等）已经完整搬进
 *     20_ReminderEngine.gs 的 Overdue 阶段，没有任何独有能力需要保留。
 *   - 12_QueryEngine.gs：删除。22_QueryEngine.gs 是完整超集（多了
 *     reminder_count/last_reminder_at 合并逻辑），12_ 没有任何独有能力。
 *   - 92_ReminderEngine.gs：删除。文件头自己写明"2026-07-03 拆分说明"
 *     和"核心判断逻辑完全没改"，是 25_ReminderEngine.gs 的逐字前身，
 *     25_ 本身都要删了，92_ 更没有保留的理由。
 *   - 05_SheetUtils.gs：删除。21_SheetUtils.gs 文件头明确写着"原
 *     05_SheetUtils.gs"，是取代关系，同一类风险，一并处理。
 *
 * 这四个文件都遵循同一个模式：新文件（21_/22_/25_→已退役/20_）的文件头
 * 都明确记录了"取代自哪个旧文件"，旧文件因此都能安全确认没有独有能力、
 * 可以直接删除，不需要逐个再单独核实功能对等性。
 */
